...
This chapter presents the derivation and evaluation of the design principles that constitute the primary contribution of this thesis. Section~\ref{ssec:derivation-methods} details the methodology used to derive the design principles from the results of the empirical study described in Chapter~\ref{sec:artefact-design}, and to evaluate and revise the resulting principles through expert interviews. The coding results and first iteration of the design principles are presented in Section~\ref{ssec:initial-design-principles}. The results of the expert interviews and the resulting revisions to the design principles are reported in Section~\ref{ssec:design-principle-revision}. Finally, Section~\ref{ssec:uncovered-design-space} addresses a portion of the design space that could not be covered empirically, discusses potential reasons for this gap, and suggests theory-based design principles to address it.

\subsection{Derivation Methodology and Expert Interviews} \label{ssec:derivation-methods}

The design principles presented in this chapter are constructed following the grounded-theory approach \parencite{Gioia2013}. The grounded theory approach was chosen because its progressive abstraction from first-order concepts to second-order themes and aggregate dimensions is uniquely suited to the derivation of design principles from empirical data. Design principles serve as the final abductive step in the grounded theory process by connecting the inductively derived data structure to the pre-existing theoretical framework of meta-requirements and design requirements established in Section~\ref{ssec:meta-requirements}.

Coding was conducted in two cycles, following the guidance in \textcite{Saldana2015}. The first cycle applied process codes \parencite{Corbin2014}, as the analysis focused on participants' interaction with the \ac{AI} system. Breaking this interaction into a sequence of discrete actions allowed identification of patterns in participants' behaviour and engagement with the system. In addition, each process code was matched with an in vivo code \parencite{Corbin2014} from the transcripts to preserve a direct connection to the underlying data for the subsequent abstraction levels. Following the first cycle coding, the raw codes were cleaned and consolidated by correcting typographical and minor wording variations without altering the underlying coding decisions. The second cycle of coding applied pattern coding \parencite{Miles2013}, combining first-cycle codes with similar or related meanings into smaller sets of pattern codes to reduce the overall code space to a size suitable for further thematic analysis.

The pattern codes produced in the second cycle were used as first-order concepts, which were then grouped into second-order themes and aggregate dimensions. The second-order themes and aggregate dimensions were then used to derive the initial design principles using the meta-requirements and design requirements defined in Section~\ref{ssec:meta-requirements} as guidance. This inductive process of stepwise abstraction allows for the constructed data structure to be grounded in the empirical data. Using the meta-requirements and design requirements as a guiding framework positions the resulting design principles as connections between the inductively derived data structure and the pre-existing theoretical framework \parencite[cf.][]{Gioia2013}. Using the same codes across both groups also allows for a limited degree of quantitative comparison, complementing the qualitative construction of design principles.

The resulting design theory was then evaluated through expert interviews with \ac{HCI} and \ac{HAII} researchers and industry practitioners. The interviews were semi-structured and aimed to evaluate each \ac{DP} individually, as well as the design theory as a whole. Interview questions were designed to evaluate the design principles against the criteria of \textbf{accessibility}, \textbf{importance}, \textbf{novelty and insightfulness}, \textbf{actability and guidance}, and \textbf{effectiveness} proposed by \textcite{Iivari2021}. The design theory as a whole was evaluated for \textbf{deficits} and \textbf{overload} \parencite{Janiesch2020}. The feedback gathered during the interviews was used to refine and revise the design principles.

\subsection{Initial Design Principles} \label{ssec:initial-design-principles}

The original transcripts contained only speaker annotations and stage directions. To better support the grounded theory data structure, they were reformatted into a semi-structured format. This assigns each statement a unique identifier, used throughout this section to reference specific statements in the transcripts. These identifiers follow the format \texttt{[participant ID].[turn ID].[statement ID]}. Details on the format, including the additional metadata it carries, are documented in the supplementary materials \parencite{Goepfert2025}. The reformatted transcripts were used to construct the Gioia data structure through the following steps:
...
The resulting seven aggregate dimensions are shortly described below, listing the contained second-order themes and highlighting representative verbs from the underlying pattern codes. Figure~\ref{fig:theme-dimension-mapping} visualises the mapping. The complete pattern-code-to-theme mapping is provided in the appendix.
...
\begin{itemize}
    ...
    \item \textbf{Information Exchange} centres on the communication between the participant and the \ac{LLM}. Its themes \textit{Query Formulation} (formulating), \textit{Information Disclosure to LLM} (submitting, stating), and \textit{LLM Response Processing} (reading, restating) cover the key steps of the interaction.
    \item \textbf{Mental Model \& Updates} groups themes focused on the participants' reception of \ac{LLM} outputs and the resulting attributions of mental states. \textit{LLM Output Evaluation} (evaluating, scrutinising), \textit{LLM Guidance Uptake} (accepting, rejecting), \textit{Identifying \& Correcting LLM Errors} (correcting, criticizing), and \textit{LLM Attributions} (attributing).
    ...
    \item \textbf{Organisation} is a single-theme dimension covering experiment logistics, tool handling, and clarification requests directed at the researcher rather than the system.
\end{itemize}
...
Dimensions vary widely in size (by the number of statements), and dimension size is not a reliable indicator of analytical importance. \textit{Problem Cognition}, for example, is the largest dimension, but its size reflects the experimental setup that allowed users to freely engage with the \ac{LLM}. The transcripts contained 1466 coded statements, with 888 from group (E), and 578 from group (B), yielding a global split of 60.6\% of statements from group (E). This split served as a baseline for comparing between-group differences in the aggregate dimensions and determining whether the \ac{CoT} manipulation affected participants' behaviour. The statement counts per aggregate dimension are shown in Table~\ref{tab:statement-counts}.
...
This section covers the evaluation of the design principles and the revisions that resulted from it. It is split into Section~\ref{sssec:interview-procedure}, which describes the interview procedure and the experts who participated. Section~\ref{sssec:interview-results} presents the key results of the evaluation, and Section~\ref{sssec:revised-dp} presents the revised, final set of design principles.

\subsubsection{Interview Procedure and Participants} \label{sssec:interview-procedure}

The six design principles derived in Section~\ref{ssec:initial-design-principles} were evaluated through semi-structured expert interviews. In total, four experts were interviewed, three of whom were \ac{HCI} and \ac{HAII} researchers, and one industry practitioner with a focus on the development of generative AI applications. Table~\ref{tab:expert-participants} lists details about all participants
...
The semi-structured format allowed for consistent evaluation of the \ac{DP} across all experts while also enabling exploration of additional, unprompted topics. The interviews were conducted online via Zoom, lasted between 60 and 75 minutes, and followed the same high-level procedure outlined below. The interviews were recorded and transcribed for analysis.

\begin{enumerate}
    \item \textbf{Consent and Demographics:} Participants were informed about the goal of the interview, the use of the collected data, and that the interview would be recorded. After obtaining consent, demographic information was collected to contextualise the expert's background and experience.
    \item \textbf{Topic Introduction:} The thesis topic and relevant concepts (\ac{ToM}, \ac{DSR}, and design principles) were introduced to the participants as needed for their individual background. The thesis methodology was explained indiscriminately to all participants.
    \item \textbf{Comprehension Check:} The meta-requirements and design requirements were presented, and their relationships explained. Before evaluating each \ac{DP}, we performed a comprehension check using a fictional example of the same \ac{DR}-mapping exercise used later for the design principles. This validated the expert's understanding of the \ac{MR}/\ac{DR}/\ac{DP} concepts and familiarised them with the exercise format before the main discussion. All four experts passed the check.
    ...
    \item \textbf{Closing Discussion:} After discussing all design principles, a final set of questions was asked to evaluate the complete set of design principles for its \textit{effectiveness} \parencite{Iivari2021} and \textit{deficit} \parencite{Janiesch2020}. A question to assess \textit{redundancy} and \textit{excess} was planned, but omitted due to an oversight in the interview guide. The interview was closed with two open questions to allow the expert to provide any additional feedback.
\end{enumerate}

\textbf{Accessibility} \parencite[cf.][]{Iivari2021} was not assessed using a dedicated question, but instead inferred from the discussion of each \ac{DP} and the expert's \ac{DR}-mapping. Two key indicators operationalised accessibility: first, a qualitative measure based on the clarification-seeking questions raised by the expert during the discussion (e.g., requesting the meaning of a term or an example). Secondly, a quantitative measure, based on the agreement between the expert's stated \ac{DR}-mapping and the mapping intended by the derivation in Section~\ref{ssec:initial-design-principles}. The criterion of \textbf{overload} \parencite[cf.][]{Janiesch2020} was not assessed directly, but an indirect signal can be inferred from the actability question, based on the variance in experts' understanding of the mechanism. The complete set of questions asked during the interviews is provided in the Appendix.

\subsubsection{Result Summary} \label{sssec:interview-results}

The interview transcripts were evaluated after the interviews by filling an evaluation form recording the \ac{DR}-mappings and responses to each question. The responses to each question were then combined into an evaluation based on the categories described in Section~\ref{sssec:interview-procedure}. Table~\ref{tab:dr-mapping-experts} shows the DR-mappings compared with the mapping proposed in Section~\ref{ssec:initial-design-principles}. Individual remarks beyond this fixed set of categories are included below only where they were raised independently by two or more experts, or where they connect directly to a construct or citation already established elsewhere in the design theory. The qualitative evaluation of each \ac{DP} is outlined below, followed by notions from the closing discussion.
...
The evaluation results show that the experts' mappings generally agree with the proposed mappings. The most notable differences are a complete misalignment of \textbf{DP-D} and the high amount of clarification required for \textbf{DP-F}. In contrast, \textbf{DP-A}, \textbf{DP-B}, and \textbf{DP-C} are well understood, while \textbf{DP-E} shows minor comprehension. Another notable difference between the experts' mappings and the proposed mappings is that the experts primarily mapped only one \ac{DR} to each \ac{DP}, indicating that the experts' understanding of the design principles was more focused than the proposed mappings.

The following section discusses the results for each \ac{DP} in detail, including the \ac{DR} mappings. Relevant remarks from the interviews are highlighted, and the implications for the design principles are discussed.

\begin{enumerate}[label=\textbf{DP-\Alph*}, leftmargin=*]
    % former DP-F
    \item \textbf{Re-use the user's own task vocabulary} \\
        Three of the four experts mapped the \ac{DP} to \textbf{DR1}, with two adding \textbf{DR5} alongside it. E04 was the outlier, mapping the \ac{DP} to \textbf{DR2}, arguing that the use of specific vocabulary creates capability expectations. This shows the \ac{DP} is very \textit{accessible}.
        The \ac{DP}-specific discussion questions showed that experts found the \ac{DP} \textit{actable} and \textit{important}, but not particularly \textit{novel}, calling it existing behaviour. During the discussion, E01 and E04 raised the risks of sycophancy and anthropomorphism, respectively. They warned that mirroring the user’s vocabulary could reduce scrutiny and increase the tendency to anthropomorphise the system. These risks are also raised in the literature \parencite{Sharma2024, Colombatto2025}, leading to the conclusion that the \ac{DP} should be revised to mitigate these risks.

    % former DP-G
    \item \textbf{Clearly state system limitations} \\
        All four experts agreed that \textbf{DP-B} addresses \textbf{DR2}, with the caveat that two required clarifications before being able to map the \ac{DP}. This speaks to good \textit{accessibility}, with improvement potential in the mechanism, which caused both clarification questions. Experts agreed that the \ac{DP}'s \textit{novelty} was limited. Experts' implementation suggestions almost all shared the idea of tweaking the vocabulary the model uses in its responses, indicating clear \textit{actability}. It will be revised by rewording the mechanism of the \ac{DP} to focus on the system's own disclosure of its limitations.

    % former DP-H
    \item \textbf{Support error detection} \\
        The \textit{actability} question revealed two directions for \textbf{DP-C}, revealing two distinct mechanisms to support error detection. The first one focused on providing sources for information, through external sources or references to the existing chat history. The second mechanism focused on surfacing conflicts between established context and new information provided by the user. Combined with the \ac{DR}-mapping showing high \textit{accessibility}, this indicated that the \ac{DP} was \textit{overloaded} and should be split into two separate principles. All experts agreed that the \ac{DP} was \textit{important}, with E01 noting that instructing the system to surface conflicts could help counteract the sycophancy risk raised in \textbf{DP-A}. Similarly, E03 noted that surfacing conflicting info might contradict the sycophantic tendencies of \acp{LLM}. All experts also rated \textit{novelty} high, with all experts noting that combining external and internal references was a new idea to them.

    % former DP-C
    \item \textbf{Make system assumptions inspectable and correctable} \\
        All experts mapped \textbf{DP-D} to \textbf{DR3}, showing uniform disagreement with the proposed mapping. The uniformity of the mapping and the lack of clarifying questions suggest that this is not an \textit{accessibility} problem. Instead, the \ac{DR}-mapping of this \ac{DP} will be reevaluated. Implementation concepts converged on highlighting assumptions in the system’s response using uncertainty as a determinant, e.g., through hedging language or confidence scores. This indicates that the \ac{DP} is \textit{actable}. However, the idea of self-reporting confidence is currently contested in the literature \parencite{Kapoor2024}.

    % former DP-A
    \item \textbf{Establish request purpose before querying} \\
        \textbf{DP-E} was mapped to \textbf{DR2} by most experts, indicating average \textit{accessibility}. A notable omission by all experts is a lack of mapping to \textbf{DR4}, as E01 and E04 both raised concerns about the friction introduced by the \ac{DP}, which could lead to habituation and reduced effectiveness over time.  All experts rated its \textit{importance} high, with E01 noting very specialised systems (e.g., interactive knowledge databases) as exceptions, where the \ac{DP} would be less important. Three of the experts said that while the underlying idea of tailoring the system's response to a given role was known to them, the specific mechanism of enforcing a role assignment was genuinely new.

    % former DP-E
    \item \textbf{Provide additional, user-owned working surfaces} \\
        \textbf{DP-F} showed the lowest \textit{accessibility} of all design principles, with three of four experts requiring clarification before they could map the \ac{DP}. The biggest challenge was the \ac{DP}'s vague wording, which created uncertainty about the intended mechanism. After clarification, experts rated the \textit{novelty} of the \ac{DP} as low, despite E01 noting that the notion that system-readable working surfaces reduce cognitive load was new to them. This indicates a need to reword the \ac{DP} using simpler language.
\end{enumerate}

During the mapping exercise, E01, E03, and E04 all asked about the term \enquote{intelligibility} and noted that it could be replaced with \enquote{understandability}. The complete set of design principles was evaluated in the categories of \textit{effectiveness} \parencite{Iivari2021} and \textit{deficit} \parencite{Janiesch2020}. Because the question on \textit{redundancy} and \textit{excess} was omitted and experts did not offer any unprompted comments, no evaluation could be performed in these categories. Experts agreed that the set of design principles would help address the design principles \textit{effectively} and improve the quality of systems built with the design principles. Experts also agreed that the language of the design principles and doubts about the full technical feasibility of the mechanisms were the main challenges to overcome when using the design principles. When asked about \textit{deficits}, E01 noted that the \ac{DP} could be complemented by addressing known cognitive biases (e.g., automation bias). While automation bias in itself is not covered in this thesis, the associated risk of heuristic acceptance \parencite{Jussupow2021} is discussed in Section~\ref{ssec:mental-state-attributions}. E02 was the only expert to ask why none of the design principles addressed the \ac{CoT} explanations directly, which will be addressed in Section~\ref{ssec:uncovered-design-space}.

\subsubsection{Revised Design Principles} \label{sssec:revised-dp}

The findings from Section~\ref{sssec:interview-results} were used to revise the existing design principles. The final DP are presented in this section using the reduced Gregor format described in Section~\ref{ssec:thesis-methodology}. Specifying the \textbf{context} of the DP is omitted because the context is identically scoped to a collaborative problem-solving setting, where a user is assisted by an explainable \ac{LLM}. Because findings on the impact of explanations are limited, the design principles do not rely on explanations as a mechanism. Each final \ac{DP} is presented below in the same format, with the title at the top followed \textbf{implementer}, \textbf{aim}, \textbf{user}, \textbf{mechanism}, and \textbf{rationale} in tabular form.

\textbf{DP-A} required substantial revision to address experts' concerns about anthropomorphic tendencies when mirroring user vocabulary. The revised \textbf{DP1} broadens the scope from vocabulary-matching to adjusting the system's response behaviour to the user's demonstrated (or stated) expertise level. Broadening the mechanism allows it to still address user-specific comprehension \parencite{Shin2021}, while avoiding prescribing potentially sycophantic behaviour \parencite{Sharma2024} and anthropomorphic tendencies \parencite[cf.][]{Weisz2025} raised by the experts. The experiment transcripts provide examples of the challenges introduced when the system's responses are not aligned with the user's expertise level (cf. ID \texttt{11.009.1} and \texttt{11.017.1}ff). The rationale for the revised \textbf{DP1} can be retained. The revised \textbf{DP1} continues to serve \textbf{DR1} and now also addresses \textbf{DR5}, as matching the user's expertise level reduces the effort required, especially for users with lower expertise, who would otherwise need to infer the meaning of technical terms and concepts.
...
Conceptually, \textbf{DP-E} remains unchanged, but the mechanism is reworded to improve \ac{DP} \textit{accessibility}. The revised mechanism focuses on establishing the collaboration mode, and the revised rationale addresses the intentionality of the friction introduced by the \ac{DP}, which was raised by the experts.
...
\textbf{DP-F} is completely reworded to address the \textit{accessibility} issues identified during the interviews. The revised \textbf{DP7} uses plainer language, omits the concept of cognitive offloading in the mechanism, and instead describes the offloading process.
...
Three design principles (\textbf{DP-B}, \textbf{DP-E}, \textbf{DP-F}) were reworded to improve \textit{accessibility}, without changing their \ac{DR}-mapping. The other three (\textbf{DP-A}, \textbf{DP-C}, \textbf{DP-D}) were revised, including an update to the \ac{DR}-mapping. Additionally, the sycophancy risk raised by the experts for \textbf{DP-A} is addressed in the revised set. Furthermore, due to unprompted expert feedback, the wording of \textbf{DR1} was also revised, replacing \enquote{intelligibility} with \enquote{understandability}. The revised set of design principles is shown in Figure~\ref{fig:mr-dr-dp-mapping-revised}, which highlights rewording changes in blue and revised the \ac{DP} in purple.
...
This section discusses the design space that was not covered by the design principles derived in Section~\ref{ssec:initial-design-principles}. Firstly, potential reasons for the lack of empirically grounded design principles are discussed in Section~\ref{sssec:gap-analysis}. Secondly, a set of design principles based on the literature discussed in Chapters~\ref{sec:fundamentals} and \ref{sec:problem-awareness} is proposed to close the identified gaps in Section~\ref{sssec:theory-dp}.

\subsubsection{Gap Analysis} \label{sssec:gap-analysis}

The design theory presented in Section~\ref{ssec:design-principle-revision} leaves two gaps in the design space. Firstly, the design principles derived from the empirical data do not address explanations directly. The only related finding is the difference in heuristic uptake of \ac{LLM} responses between the two groups. Second, \textbf{DR2} is addressed only one-sidedly with \textbf{DP2} prescribing the disclosure of limitations. \textbf{DP6} also addressed \textbf{DR2}, but does so by setting capability expectations, not by proactively communicating capabilities. Several potential reasons for these gaps are discussed below.

The first potential reason is that the task complexity was too low to prompt explanation engagement. This is supported by the coding results showing that participants' effort was focused on solving the problems themselves rather than engaging with the \ac{LLM}: \textit{Problem Cognition} is the largest aggregate dimension by statement count. At the chosen difficulty level, engaging with the \ac{LLM} and the explanations (if provided) was optional rather than necessary, leading to the small number of explanation-engagements for group (E). This hypothesis is also supported by the fact that 32 of 36 tasks were solved correctly. Of the remaining four tasks, two were solved incorrectly, and two were aborted and replaced with a new task. Low engagement with explanations may also be traced to the UI design, which rendered the explanations collapsed by default. P04 noted during the debriefing that explanations should be expanded by default.
...
\begin{ctable}
    ...
    \begin{tabular}{l c c c}
        ...
        \bottomrule
    \end{tabular}
    \caption[Quantitative results of the study]{Quantitative comparison of group (B) and group (E). Arrows indicate the direction in which the explanation-augmented condition (group E) was hypothesised to differ in the original experiment \parencite{Goepfert2025}. Constructs marked with * are significant at $\alpha=0.1$.}
    \label{tab:quant-results-gap}
\end{ctable}

Only two constructs show significant differences. \textbf{Frustration} shows a large, statistically significant effect. However, it is a workload construct and reflects how the interaction felt, not the user's mental model. \textbf{Perceived usefulness}, which is an acceptance construct, shows a smaller effect and is only significant at $\alpha=0.1$. Because it reflects the user's perception of the system's utility, it is affected by the user's mental model. Both effects indicate that the explanation-augmented condition had an effect, with \textbf{perceived usefulness} even being compatible with the silent-reading hypothesis. Due to the confounding effect of improved response quality (cf. Section~\ref{ssec:chatbot-implementation}), the effect cannot be attributed to explanation presence alone. The remaining constructs show no significant differences, but the workload constructs show a directional effect, with group (E) reporting lower workload than group (B). The two remaining acceptance and efficacy constructs show effects in the opposite direction of the hypothesis. For \textbf{Perceived Ease of use} this could be explained by the additional interface element present only for group (E). \textbf{Computer self-efficacy} measures a dispositional trait, which could indicate imperfect group equivalence rather than a result of the experiment.

\subsubsection{Theory-derived Design Principles} \label{sssec:theory-dp}

This section proposes design principles derived from the literature reviewed in Chapters~\ref{sec:fundamentals} and \ref{sec:problem-awareness}, to close the gaps identified above. Because the design principles were developed after the expert interviews concluded, they are not evaluated in the same way as \textbf{DP1}~--~\textbf{DP7} and are numbered separately to avoid ambiguity. The same modified Gregor format is used, and mappings to the design requirements are provided.

\textbf{DR1} calls for the system to be understandable, with \textbf{DP1}, \textbf{DP3}, and \textbf{DP5} addressing aspects of the requirement. \textbf{DP3} and \textbf{DP5} address the \textit{knowledge distribution} aspect of mental models, and \textbf{DP2} and \textbf{DP-T5} seek to influence the model of \textit{global behaviour} \parencite{Gero2020}. None of the design principles addresses the \textit{local behaviour} aspect, which is particularly important for generative systems, where the same request can yield materially different outputs on repeated invocation \parencite{Weisz2025}. Therefore, \textbf{DP-T1} addresses this gap. Providing local explanations addresses \textbf{DR1}. If the explanations are faithful, they can also support \textbf{DR3}, as they allow the user to verify the system's reasoning process.
...
\ac{CoT} explanations are not guaranteed to reflect a system's underlying reasoning process; i.e., they can be unfaithful to the system’s reasoning process \parencite{Lyu2023}. An unfaithful explanation can corrupt the user's mental model (cf. Figure~\ref{fig:genai-challenges-chain}) and therefore lead to miscalibrated reliance on the system's output \parencite{Jussupow2021}. Because guaranteeing faithfulness is not a solved problem \parencite{Tutek2025}, \textbf{DP-T2} aims to mitigate the risk of unfaithful explanations. By treating explanations as challengeable evidence, this \ac{DP} supports \textbf{DR1} and \textbf{DR3}, even where faithfulness cannot be guaranteed.
...
The second gap identified in Section~\ref{sssec:gap-analysis} was the lack of design principles addressing the communication of system capabilities. \textbf{DP-T5} addresses this gap building, on G1 and G2 by \textcite{Amershi2019}, which call for disclosing the system's capabilities. A key reason this was not addressed in the empirical data is that capability communication should happen \enquote{initially} \parencite{Amershi2019}. The experiment design included an initial briefing, which was not captured in the transcripts and therefore not coded. The \ac{DP} complements \textbf{DP2} by proactively communicating capabilities. The \ac{DP} addresses \textbf{DR2} by ensuring that users are aware of the system's capabilities.
...
Beyond the five principles above, \textbf{DP1} can be extended to cover explanations specifically. The same grounding already cited for \textbf{DP1} shows that a given explanation can achieve different levels of causability for different users \parencite{Shin2021}, and tailoring explanations to the user aligns with the audience-specific definition of \ac{XAI} adopted in Section~\ref{sssec:terminology} \parencite{Arrieta2020}.

Figure~\ref{fig:mr-dr-dp-mapping-final} combines the seven empirically grounded and expert-evaluated principles from Section~\ref{sssec:revised-dp} with the five theory-derived principles proposed above.
...
\begin{figure}[ht]
    ...
    \caption[Design theory and theory-derived \ac{DP}]{Design theory and theory-derived \ac{DP}. Design principles are organised to reduce the number of edge crossings. Theory-derived principles are highlighted in blue.}
