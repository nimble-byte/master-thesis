...
This section covers the evaluation of the \acp{DP} and the revisions that resulted from it. It is split into Section~\ref{sssec:interview-procedure}, which describes the interview procedure and the experts who participated. Section~\ref{sssec:interview-results} presents the key results of the evaluation, and Section~\ref{sssec:revised-dp} presents the revised, final set of \acp{DP}.
...
\begin{enumerate}
    ...
    \item \textbf{Comprehension Check:} The \acp{MR} and \acp{DR} were presented, and their relationships explained. Before evaluating each \ac{DP}, we performed a comprehension check using a fictional example of the same \ac{DR}-mapping exercise used later for the \acp{DP}. This validated the expert's understanding of the \ac{MR}/\ac{DR}/\ac{DP} concepts and familiarised them with the exercise format before the main discussion. All four experts passed the check.
...
\begin{enumerate}[label=\textbf{DP-\Alph*}, leftmargin=*]
    ...
    \item \textbf{Provide additional, user-owned working surfaces} \\
        \textbf{DP-F} showed the lowest \textit{accessibility} of all \acp{DP}, with three of four experts requiring clarification before they could map the \ac{DP}. The biggest challenge was the \ac{DP}'s vague wording, which created uncertainty about the intended mechanism. After clarification, experts rated the \textit{novelty} of the \ac{DP} as low, despite E01 noting that the notion that system-readable working surfaces reduce cognitive load was new to them. This indicates a need to reword the \ac{DP} using simpler language.
\end{enumerate}

During the mapping exercise, E01, E03, and E04 all asked about the term \enquote{intelligibility} and noted that it could be replaced with \enquote{understandability}. The complete set of \acp{DP} was evaluated in the categories of \textit{effectiveness} \parencite{Iivari2021} and \textit{deficit} \parencite{Janiesch2020}. Because the question on \textit{redundancy} and \textit{excess} was omitted and experts did not offer any unprompted comments, no evaluation could be performed in these categories. Experts agreed that the set of \acp{DP} would help address the \acp{DP} \textit{effectively} and improve the quality of systems built with the \acp{DP}. Experts also agreed that the language of the \acp{DP} and doubts about the full technical feasibility of the mechanisms were the main challenges to overcome when using the \acp{DP}. When asked about \textit{deficits}, E01 noted that the \ac{DP} could be complemented by addressing known cognitive biases (e.g., automation bias). While automation bias in itself is not covered in this thesis, the associated risk of heuristic acceptance \parencite{Jussupow2021} is discussed in Section~\ref{ssec:mental-state-attributions}. E02 was the only expert to ask why none of the \acp{DP} addressed the \ac{CoT} explanations directly, which will be addressed in Section~\ref{ssec:uncovered-design-space}.

\subsubsection{Revised Design Principles} \label{sssec:revised-dp}

The findings from Section~\ref{sssec:interview-results} were used to revise the existing \acp{DP}. The final DP are presented in this section using the reduced Gregor format described in Section~\ref{ssec:thesis-methodology}. Specifying the \textbf{context} of the DP is omitted because the context is identically scoped to a collaborative problem-solving setting, where a user is assisted by an explainable \ac{LLM}. Because findings on the impact of explanations are limited, the \acp{DP} do not rely on explanations as a mechanism. Each final \ac{DP} is presented below in the same format, with the title at the top followed \textbf{implementer}, \textbf{aim}, \textbf{user}, \textbf{mechanism}, and \textbf{rationale} in tabular form.
...
Three \acp{DP} (\textbf{DP-B}, \textbf{DP-E}, \textbf{DP-F}) were reworded to improve \textit{accessibility}, without changing their \ac{DR}-mapping. The other three (\textbf{DP-A}, \textbf{DP-C}, \textbf{DP-D}) were revised, including an update to the \ac{DR}-mapping. Additionally, the sycophancy risk raised by the experts for \textbf{DP-A} is addressed in the revised set. Furthermore, due to unprompted expert feedback, the wording of \textbf{DR1} was also revised, replacing \enquote{intelligibility} with \enquote{understandability}. The revised set of \acp{DP} is shown in Figure~\ref{fig:mr-dr-dp-mapping-revised}, which highlights rewording changes in blue and revised the \ac{DP} in purple.
...
The design theory presented in Section~\ref{ssec:design-principle-revision} leaves two gaps in the design space. Firstly, the \acp{DP} derived from the empirical data do not address explanations directly. The only related finding is the difference in heuristic uptake of \ac{LLM} responses between the two groups. Second, \textbf{DR2} is addressed only one-sidedly with \textbf{DP2} prescribing the disclosure of limitations. \textbf{DP6} also addressed \textbf{DR2}, but does so by setting capability expectations, not by proactively communicating capabilities. Several potential reasons for these gaps are discussed below.
...
\begin{figure}[ht]
    ...
    \caption[Design theory and theory-derived \ac{DP}]{Design theory and theory-derived \ac{DP}. \acp{DP} are organised to reduce the number of edge crossings. Theory-derived principles are highlighted in blue.}
