...
The original transcripts from the study only contained speaker annotations and stage directions, to better support the construction of the grounded theory data structure. The transcripts reformatted into a semi-structured format, that allowed each statement to be assigned a unique identifier, that can be used to reference the statement throughout the coding process. These identifiers have the format \texttt{[participant ID].[turn ID].[statement ID]} and will be used throughout this section to reference specific statements in the transcripts. Details about the format including the additional metadata are documented in the supplementary materials \parencite{PLACEHOLDER}.
The reformatted transcripts were then used to construct the Gioia data with the following steps: % This is terrible wording
...
\begin{enumerate}[label=\textbf{DP-\Alph*}, leftmargin=*]
    ...
    \item \textbf{Clearly state system capabilities and limitations}, clarify the system's capabilities and particularly its limitations to the user during the conversation. This \ac{DP} is motivated by the observation that participants attributed capabilities to the system, without factual basis (pattern code \textit{attributing capability to LLM}). A good example is the expectation, that the \ac{LLM} draws the same conclusion from the provided image: \enquote{aber eigentlich hat er ja das Bild} [but he does have the image] (ID: 08.14.1). The attempts to get the \ac{LLM} to solve the maths problems are a prominent example, where the system should clearly state its limitations. By clearly communicating capabilities and limitations, the user expectations about system capabilities are managed fulfilling \textbf{DR2}.

    % This DP has been significantly shortened; the plan is to present it as it was conceived and to split it with 6.3, which is more accurate anyways.
    \item \textbf{Support error detection}, focuses on supporting the user in detecting and attributing errors. This \ac{DP} is anchored in the observation, that users accepted system advice heuristically (cf. \textbf{DP-A} rationale). Highlighting the source (e.g. task given, user-provided, externally-retrieved, etc.) of a piece of information in the system response could invite more scrutiny by the users. Additionally, the \ac{LLM} also accepted user input, even if it conflicted with previous inputs. The most prominent example of this behaviour is, when the system adopted a sign error by P08 signalling endorsement \parencite[cf.][]{Sharma2024} of the change. The participant later explicitly commented \enquote{Schade, dass er solche Logikfehler nicht erkennt.} [Shame he doesn't recognise such logical errors.] (ID: \texttt{08.048.6}). This design principle serves to fulfil \textbf{DR1} and \textbf{DR3} by providing sources for information, and \textbf{DR4} by surfacing conflicting information to the user.
\end{enumerate}

% TODO: This observation needs to go or at least be re-written as it is concerned by implementation details. It likely can be combined with the last paragraph and have the figure close the section.
...
% TODO: The arrows from DR to DP need to be replaced by coloured lines. That means we need to introduce two additional colours that are distinct enough compared to the existing ones.
\begin{figure}[ht]
    \centering
    \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
        ...
        \node[dpbox] (dpe) at (17,0.3)  {\textbf{DP-E:}~Provide additional, user-owned working surfaces};
        \node[dpbox] (dpf) at (17,-1.3) {\textbf{DP-F:}~Re-use the user's own task vocabulary};
        \node[dpbox] (dpg) at (17,-2.7) {\textbf{DP-G:}~Clearly state system capabilities and limitations};
        \node[dpbox] (dph) at (17,-4.2) {\textbf{DP-H:}~Support error detection};
...
% TODO: Combine with summary paragraph before the figure; close that with reference to the figure
