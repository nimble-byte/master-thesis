...
\designprinciple{DP1}{Adapt responses to user expertise}
    ...
    {Because matching the user's expertise increases the intelligibility of the system's responses.}

\textbf{DP-B} is functionally unchanged, but the mechanism is reworded, to improve the \textit{accessibility} of the \ac{DP}. The revised mechanism shifts the emphasis from the user's inference of system capabilities to the system's own disclosure of its capabilities and limitations. The rationale is unchanged, as managing user expectations about system capabilities reduces the likelihood of over- or under-reliance \parencite[cf.][]{Jussupow2021}. With the revised mechanism, the \ac{DP} continues to serve \textbf{DR2}.

\designprinciple{DP2}{Disclose system capabilities and limitations}
    {For the \textit{developer} to tune the system to \textit{disclose its capabilities and limitations} to \textit{users} during the interaction}
...
\textbf{DP-C} is split into two separate principles to address the two distinct mechanisms identified through the interviews. The new \textbf{DP3} focuses on the information provenance mechanism, that provides sources for information, while the new \textbf{DP4} describes the surfacing of conflicts between established context and new user input. The \ac{DR}-mapping of the new principles still covers the same \acp{DR} as the original \textbf{DP-C}, but now with a more focused mechanism for each principle. \textbf{DP3} now helps fulfil \textbf{DR1} and \textbf{DR3}, as providing sources for information, allows users to better understand the system's use of information. \textbf{DP4} serves \textbf{DR4}, as surfacing conflicts between established context and new user input increases deliberate engagement.

\designprinciple{DP3}{Indicate information provenance}
    {For the \textit{system} to \textit{increase the transparency of its information use} for \textit{users}}
    {provide the \textit{source of each piece of information} in the system's response (e.g., user-provided, system-assumed, or externally retrieved).}
    {Because indicating the source of information invites scrutiny of system-attributed claims rather than heuristic acceptance, increasing both the intelligibility of the system's use of information and the verifiability of individual claims}

\designprinciple{DP4}{Surface conflicting information}
    {For the \textit{system} to \textit{support the detection of errors} for \textit{users}}
...
\textbf{DP-E} is kept intact as initially proposed. The concern about habituation raised by two experts are acknowledged, but the mechanism is kept unchanged, as the goal of the \ac{DP} is to increase friction to generate deliberate engagement. Possible habituation risks, have to be assessed on a case-by-case basis, as they depend on the specific implementation of the mechanism.

% keep: refine mechanism against habituation risk
\designprinciple{DP6}{Establish request purpose before querying}
    {For the \textit{developer} to \textit{tune model behaviour} to for a \textit{user}'s needs}
...
% TODO: This section needs some form of summary or anything to close out this section.
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        \node[dpbox] (dp2)  at (17,2.6)  {\textbf{DP2:}~Disclose system capabilities and limitations};
        ...
        \node[font=\bfseries] at (8.5,5.0) {Design Requirements};
    ...
    % TODO: implement highlighting for revised DPs in blue (border + background)
    \caption[Revised design principles]{The revised design principles mapped onto the design requirements which they fulfil. Adjusted \ac{DP} are highlighted in blue.}
